...
router.get("/get-imp-tasks", authenticateToken, async (req, res) => {
    try {
      ...
      res.status(201).json({ message: "Immportant task fetching done!", data: impTaskData });
    } catch (err) {
      console.error("Error in fetching task:", err);
      res
        .status(500)
        .json({ message: "Error during fetching important task", error: err.message });
    }
  });
...
router.get("/get-complete-tasks", authenticateToken, async (req, res) => {
    try {
      ...
      res.status(201).json({ message: "task fetching done!", data: completeTaskData });
    } catch (err) {
      console.error("Error in fetching task:", err);
      res
        .status(500)
        .json({ message: "Error during fetching complete task", error: err.message });
    }
});  
 ...
 router.get("/get-incomplete-tasks", authenticateToken, async (req, res) => {
    try {
      ...
      res.status(201).json({ message: "task fetching done!", data: incompleteTaskData });
    } catch (err) {
      console.error("Error in fetching task:", err);
      res
        .status(500)
